...
export default function ArtistForm({
    ...
    const addprofilepicture = (e) => {
        setFormData((prev) => ({
            ...prev,
            artists: prev.artists.map((artist) =>
                artist.index === index ? { ...artist, profilePicture: e.target.files[0] } : artist
            ),
        }));
    };
    ...
    const toggleshutter = () => {
        setFormData((prev) => ({
            ...prev,
            artists: prev.artists.map((artist) =>
                artist.index === index ? { ...artist, isActive: !artist.isActive } : artist
            ),
        }));
    };
            ...
            <div className="dropdown-content">
                <div className="card-container">
                    ...
                    <div className="form-grid">
                            ...
                            {ArtistData.profilePicture ? (
                                <div className="media-item">
                                    <img
                                        ...
                                        onClick={() => setSelectedImage(URL.createObjectURL(ArtistData.profilePicture))}
                                    />
                                    <button
                                        type="button"
                        ...
                        <input name="name" value={ArtistData["name"]} placeholder="Full Name (Required)" required className="input-text" onChange={handleInputChange} />
                        <input name="instrument" value={ArtistData["instrument"]} placeholder="Instrument (Required)" required className="input-text" onChange={handleInputChange} />
                        <textarea name="detail" value={ArtistData["detail"]} placeholder="Detail (Optional)" className="textarea" onChange={handleInputChange} />
                        <MediaUploadPage
                        ...
                        > Save </button>
                        <button type="button" className="remove-button" onClick={handleremoveArtist}> Remove </button>
                    </div>
                </div>
            </div>
        </>
