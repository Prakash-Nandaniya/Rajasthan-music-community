    ...
    const checkGroupName = async (groupName, community) => {
        try {
            const response = await fetch(
                `http://127.0.0.1:8000/groupNameCheck?groupName=${groupName}&community=${community}`,
                { method: "GET" }
            );
            if (!response.ok) throw new Error("Error checking group name");
            const data = await response.json();
            if (data.exists) {
    ...
    return (
        <form className="edit-group-form" >
            ...
            <div className="edit-group-form-container">
                <section className="edit-group-form-section">
                    <h2>Group Details</h2>
                    <SiteDetailsPage
                        formData={formData}
